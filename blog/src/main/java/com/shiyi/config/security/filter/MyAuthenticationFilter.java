...
import com.shiyi.utils.MultiReadHttpServletResponse;
...
//                Claims claims = Jwts.parser().setSigningKey(Constants.SALT).parseClaimsJws(jwtToken.replace("Bearer", "")).getBody();
                // 获取当前登录用户名
                System.out.println("获取当前登录用户名: " + userName);
                String token = redisCache.getCacheObject(RedisConstants.LOGIN_PREFIX + userName);
                if (StringUtils.isBlank(token)) throw new BadCredentialsException("TOKEN已过期，请重新登录!");
                if(jwtToken.equals(token)){
                    throw new BadCredentialsException("TOKEN校验失败,请重试!");
                }
                // 检查token
                SecurityUser securityUser = userDetailsService.getUserByName(userName);
                if (securityUser == null || securityUser.getCurrentUserInfo() == null) {
                    throw new BadCredentialsException("TOKEN已过期，请重新登录!");
                }
                UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(securityUser, null, securityUser.getAuthorities());
                //重置令牌时间
                redisCache.expire(RedisConstants.LOGIN_PREFIX + userName,1, TimeUnit.HOURS);
                // 全局注入角色权限信息和登录用户基本信息
